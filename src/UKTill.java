...
public class UKTill {
    ...
    public DenominationFloat[] getContents() {
        return contents;
    }
    ...
    public DenominationFloat[] getChange(int changeDue) {
        DenominationFloat[] change = new DenominationFloat[max];
        do {
            for (int i = contents.length-1; i>=0; i-- ) {
                if (contents[i].getQuantity() != 0) {
                    if (contents[i].getType().getValue() <= changeDue) {
                        do {
                            changeDue -= contents[i].getType().getValue();
                            contents[i].setQuantity(contents[i].getQuantity() - 1);
                            DenominationFloat temp = new DenominationFloat(contents[i].getType());
                            temp.setQuantity(temp.getQuantity()+1);
                            //temp(contents[i].getType());
                            //temp.setQuantity(1);
                            //addChange(temp, change);
                            addChange(temp, change);
                        } while (contents[i].getType().getValue() < changeDue);
                    }
                }
            }
        }while(changeDue !=0);

        // Null added so that the class compiles
        // ENTER CODE HERE
    ...
    public void addChange(DenominationFloat d, DenominationFloat[] change){
        if(numOfFloats<max) {
            change[numOfFloats] = d;
            numOfFloats++;
        }
        else {}
